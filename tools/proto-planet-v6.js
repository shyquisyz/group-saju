/* [2026-09-17 U2] 시안 6판 — 앱이 읽지 않는다. 5판 영역 구조 + assets/planet-tex/*.jpg(이미지 생성·이음새 제거) 삼면 투영.
   사용: await loadPlanetTex("assets/planet-tex/"); renderPlanet6(개수, 씨앗, 관계계열, 세기). 결과: docs/reports/img/2026-09-17-U2/8-*.jpg */
/* 시안 6(5판 구조 + 생성 표면 그림): 시안 5: 표면을 큰 영역으로 나눈다(상위 3행, 많을수록 넓게). 영역마다 그 오행의 모습으로 —
...
(function(){
  ...
  function tri(k,x,y,z,sc){
    let wx=Math.abs(x)**4,wy=Math.abs(y)**4,wz=Math.abs(z)**4;const s=wx+wy+wz;wx/=s;wy/=s;wz/=s;
  ...
  window.makePlanet6=function(cnt,seed){
    ...
    const TSC={mok:1.2,su:1.0,hwa:.85,to:.5,geum:1.35};
    const tex=paint(1024,512,(x,y,z)=>{
      ...
      }else{c=mix(c,[.2,.22,.26],.5);gm=sstep(.02,.03,m);r=.5;}
    ...
    grp.add(new THREE.Mesh(geo,new THREE.MeshStandardMaterial({map:tex.map,bumpMap:tex.map,bumpScale:.02,emissiveMap:tex.emi,emissive:0xffffff,emissiveIntensity:1.3,roughnessMap:tex.rough,roughness:1,metalness:0})));
  ...
  function addTail(ps,level,col){
    ...
    [[.26,0,1],[.1,.15,.5],[.1,-.15,.5],[.05,.27,.3],[.05,-.25,.3]].forEach(([w,off,o],i)=>{
  ...
  window.renderPlanet6=function(cnt,seed,rel,level){
    ...
    const {grp}=makePlanet6(cnt,seed);grp.rotation.set(.3,seed*.9,.1);ps.add(grp);
