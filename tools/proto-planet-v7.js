/* 시안 7(약점 제거: 사막 그림 교체·금 영역 표면화·혜성 꼬리) ← 시안 6(5판 구조 + 생성 표면 그림): 시안 5: 표면을 큰 영역으로 나눈다(상위 3행, 많을수록 넓게). 영역마다 그 오행의 모습으로 —
...
(function(){
  ...
  function paint(TW,TH,f){
    const out={};const cvs={};
    ['map','emi','rough','geum','hwa','metal'].forEach(n=>{const c=document.createElement('canvas');c.width=TW;c.height=TH;const x=c.getContext('2d');cvs[n]={c,x,im:x.createImageData(TW,TH)};});
    for(let py=0;py<TH;py++){const phi=py/TH*Math.PI,y=Math.cos(phi),sp=Math.sin(phi);
      for(let px=0;px<TW;px++){const th=px/TW*Math.PI*2,x=-Math.cos(th)*sp,z=Math.sin(th)*sp;
        ...
        put('map',o.c);put('emi',o.e||[0,0,0]);put('rough',[o.r,o.r,o.r]);put('geum',[o.g,o.g,o.g]);put('hwa',[o.h,o.h,o.h]);const mt=o.mt||0;put('metal',[mt,mt,mt]);}}
  ...
  /* 오행별 표면 그림(이미지 생성, 이음새 없이 다듬음) — 픽셀 배열로 들고 삼면 투영으로 읽는다 */
  const TEX={};
  window.loadPlanetTex=base=>Promise.all(['mok','su','hwa','to','geum'].map(k=>new Promise((res,rej)=>{
    const im=new Image();im.onload=()=>{const c=document.createElement('canvas');c.width=im.width;c.height=im.height;
      const x=c.getContext('2d');x.drawImage(im,0,0);TEX[k]={w:im.width,h:im.height,d:x.getImageData(0,0,im.width,im.height).data};res();};
    im.onerror=rej;im.src=base+k+'.jpg';})));
  function px(k,u,v){const t=TEX[k];let X=Math.floor((u-Math.floor(u))*t.w),Y=Math.floor((v-Math.floor(v))*t.h);const i=(Y*t.w+X)*4;
    return [t.d[i]/255,t.d[i+1]/255,t.d[i+2]/255];}
  function tri(k,x,y,z,sc){
    let wx=Math.abs(x)**12,wy=Math.abs(y)**12,wz=Math.abs(z)**12;const s=wx+wy+wz;wx/=s;wy/=s;wz/=s;
    const a=px(k,y*sc+.37,z*sc+.11),b=px(k,x*sc+.73,z*sc+.29),c=px(k,x*sc+.19,y*sc+.53);
    return [a[0]*wx+b[0]*wy+c[0]*wz,a[1]*wx+b[1]*wy+c[1]*wz,a[2]*wx+b[2]*wy+c[2]*wz];}
  window.makePlanet7=function(cnt,seed){
    ...
    const SC=1.35;
    /* 그림 반복 크기 — 사막 층·용암 웅덩이는 크게 보여야 읽힌다(실측: 1.35에서는 200px 초상에서 층이 안 보임) */
    const geumMain=R.els[0]==='geum';
    const TSC={mok:1.2,su:1.0,hwa:.85,to:.8,geum:1.6};
    const tex=paint(1024,512,(x,y,z)=>{
      const g=R.at(x,y,z),k=sunMode?'hwa':g.k,m=sunMode?1:g.m,det=D(x*7,y*7,z*7,3);
      let c=tri(k,x,y,z,TSC[k]),e=null,r=.9,gm=0,hm=0;
      if(k==='su'){
        c=mix(c,[c[0]*.55,c[1]*.6,c[2]*.75],sstep(.05,.3,m));          // 먼 바다일수록 깊게
        c=mix([.45,.78,.8],c,sstep(0,.04,m));                          // 해안 얕은 물
        c=mix(c,[1,1,1],sstep(.955,.99,Math.abs(y)+det*.05)*.8);r=.25;
      }else if(k==='mok'||k==='to'){
        if(g.k2==='su')c=mix([.86,.8,.62],c,sstep(0,.03,m));            // 모래톱
        r=.95;
      }else if(k==='hwa'){
        const glow=sstep(.5,.85,c[0])*sstep(.2,.5,c[0]-c[2]);
        e=[c[0]*glow*1.2,c[1]*glow*1.1,c[2]*glow];
        if(!sunMode){const edge=1-sstep(0,.06,m);const ember=edge*sstep(.2,.45,D(x*9,y*9,z*9,2));
          e=[e[0]+ember,e[1]+ember*.45,e[2]+ember*.1];}
        hm=sunMode?1:sstep(0,.05,m);r=.9;
      }else{
        /* [7판] 금이 압도적이 아니면 따로 붙인 결정 덩이 대신 표면 자체를 금속으로 — 경계가 뚝 끊기지 않게 바위로 잇는다 */
        /* 적게 섞인 금 = 주변에서 솟은 은빛 산맥. 기슭은 그 자리 이웃 지형(k2)의 그림에서 바위색으로 넘어간다 */
        const t=sstep(0,.08,m);
        const foot=g.k2&&g.k2!=='geum'?tri(g.k2,x,y,z,TSC[g.k2]):[.4,.4,.42];
        const ridge=1-Math.abs(T(x*5,y*5,z*5,4));
        const peak=mix([.55,.57,.6],[.93,.95,.98],sstep(.6,.9,ridge));
        c=mix(mix(foot,[.45,.45,.47],.5),mix(c,peak,.55),t);
        r=mix([.85],[.35],t)[0];var mt=.35*t;gm=geumMain?sstep(.02,.03,m):0;}
      if(!sunMode&&k!=='hwa'&&g.k2==='hwa'){const burn=1-sstep(0,.05,m);c=mix(c,[.1,.05,.03],burn*.85);
        const ember=burn*sstep(.3,.5,D(x*9,y*9,z*9,2));e=[ember,ember*.4,ember*.08];}
      return {c,e,r,g:gm,h:hm,mt:(k==='geum'&&typeof mt!=='undefined')?mt:0};});
    ...
    for(let i=0;i<pos.count;i++){const x=pos.getX(i),y=pos.getY(i),z=pos.getZ(i);
      ...
      else if(k==='geum')h=1+(R.els[0]==='geum'?sstep(0,.04,g.m)*.006:sstep(0,.1,g.m)*(.02+(1-Math.abs(T(x*5,y*5,z*5,4)))*.025));
      pos.setXYZ(i,x*h,y*h,z*h);}
    geo.computeVertexNormals();
    grp.add(new THREE.Mesh(geo,new THREE.MeshStandardMaterial({map:tex.map,bumpMap:tex.map,bumpScale:.02,emissiveMap:tex.emi,emissive:0xffffff,emissiveIntensity:1.3,roughnessMap:tex.rough,roughness:1,metalnessMap:tex.metal,metalness:1,envMap:studioEnv(),envMapIntensity:.3})));
    ...
    if(geumMain&&!sunMode){
      ...
      const keep=[],kcol=[];const c3=new THREE.Vector3(),a3=new THREE.Vector3(),b3=new THREE.Vector3();
      for(let i=0;i<fp.count;i+=3){a3.fromBufferAttribute(fp,i);b3.fromBufferAttribute(fp,i+1);c3.fromBufferAttribute(fp,i+2);
        ...
        const g=R.at(cx/l,cy/l,cz/l);if(g.k==='geum'&&g.m>.015){keep.push(a3.x,a3.y,a3.z,b3.x,b3.y,b3.z,c3.x,c3.y,c3.z);
          const cc=tri('geum',cx/l,cy/l,cz/l,SC*3);for(let q=0;q<3;q++)kcol.push(cc[0],cc[1],cc[2]);}}
      const kg=new THREE.BufferGeometry();kg.setAttribute('position',new THREE.Float32BufferAttribute(keep,3));kg.setAttribute('color',new THREE.Float32BufferAttribute(kcol,3));kg.computeVertexNormals();
      grp.add(new THREE.Mesh(kg,new THREE.MeshStandardMaterial({vertexColors:true,metalness:.75,roughness:.22,flatShading:true,envMap:studioEnv(),envMapIntensity:1.3})));}
    // 목: 나무 빽빽한 섬
    if(R.els.includes('mok')&&!sunMode){
      const tr=rng(seed*5+3),pts=[];const share=R.target[R.els.indexOf('mok')];const want=Math.round(2600*share);
  ...
  function fanTail(len,w0,w1,col,op){
    const g=new THREE.BufferGeometry();
    const P=[],U=[],I=[];const SEG=24;
    for(let i=0;i<=SEG;i++){const t=i/SEG;const w=w0+(w1-w0)*Math.pow(t,.8);const y=t*len;const bend=Math.pow(t,2)*len*.08;
      P.push(-w/2+bend,y,0, w/2+bend,y,0);U.push(0,t,1,t);
    ...
    return new THREE.Mesh(g,new THREE.ShaderMaterial({uniforms:{c:{value:new THREE.Color(col)},op:{value:op}},
      vertexShader:'varying vec2 vU;void main(){vU=uv;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}',
      fragmentShader:`uniform vec3 c;uniform float op;varying vec2 vU;
        float h(float x){return fract(sin(x*91.7)*43758.5);}
        void main(){float side=1.-pow(abs(vU.x-.5)*2.,1.6);float along=pow(1.-vU.y,1.7)*smoothstep(0.,.06,vU.y);
          float str=.75+.25*sin(vU.x*40.+vU.y*6.);gl_FragColor=vec4(c*1.2,op*side*along*str);}`,
      transparent:true,blending:THREE.AdditiveBlending,depthWrite:false,side:THREE.DoubleSide}));}
  function addTail(ps,level,col){
    const L=[0,1.5,2.7,3.9][level];if(!L)return;
    const holder=new THREE.Group();holder.rotation.z=Math.PI*.78;
    const fan=fanTail(L,.35,.35+.25*level,col,.62);fan.position.y=.62;holder.add(fan);
    const core=fanTail(L*.8,.12,.18+.06*level,0xffffff,.45);core.position.y=.66;holder.add(core);
    const n=Math.round(70*level),pg=new THREE.BufferGeometry(),arr=new Float32Array(n*3),r=rng(level*7+3);
    for(let i=0;i<n;i++){const t=Math.pow(r(),.8);arr[i*3]=(r()-.5)*(.2+t*(.35+.25*level))+t*t*L*.08;arr[i*3+1]=.7+t*L;arr[i*3+2]=(r()-.5)*.2;}
    pg.setAttribute('position',new THREE.BufferAttribute(arr,3));
    holder.add(new THREE.Points(pg,new THREE.PointsMaterial({color:col,size:.03,transparent:true,opacity:.55,blending:THREE.AdditiveBlending,depthWrite:false})));
    holder.position.z=-.3;ps.add(holder);}

  window.renderPlanet7=function(cnt,seed,rel,level){
    ...
    const {grp}=makePlanet7(cnt,seed);grp.rotation.set(.3,seed*.9,.1);ps.add(grp);
