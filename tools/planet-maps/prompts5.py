...
# 바꾼 것: 하나의 세계 · 통일된 색조 · 나 지역 = 가장 큰 한 덩어리(약 40%) · 나머지는 실제 지형처럼 얽힘 ·
...
def prompt(me,env):
    share='about 20 percent each' if len(env)==3 else 'about 30 percent each'
    links='; '.join(LINK[frozenset(p)] for p in itertools.combinations((me,)+env,2))
    c=(f'The largest feature, about 40 percent of the surface, is one connected region of {D[me]}. '
