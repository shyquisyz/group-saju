/* [2026-09-19] map4/facing.json 생성기 — 앱(localhost)의 콘솔에서 붙여 넣어 돌린다(THREE·pLoadImg·V2_TUNABLES를 앱에서 빌린다).
...
for(const key of keys){const im=await pLoadImg('assets/planet-tex/map4/'+key+'.jpg');const W=128,H=64;const cv=document.createElement('canvas');cv.width=W;cv.height=H;const x=cv.getContext('2d');x.drawImage(im,0,0,W,H);const d=x.getImageData(0,0,W,H).data;
 ...
 for(let yaw=0;yaw<360;yaw+=10)for(const pitch of pitches){const q=new THREE.Quaternion().setFromEuler(new THREE.Euler(pitch,yaw*Math.PI/180,.08)).invert();
  ...
  const f=pi.map(e=>(s[e]||0)/t);cands.push({pitch,yaw,f,score:Math.min(...f)+.1*f[0]});}
 cands.sort((a,b)=>b.score-a.score);const best=cands[0];
 out[key]={pitch:best.pitch,yaw:best.yaw,vis:Object.fromEntries(parts.map((e,i)=>[e,Math.round(best.f[i]*100)])),pass:cands.filter(c=>c.score>=best.score*.93).map(c=>[c.pitch,c.yaw])};}
